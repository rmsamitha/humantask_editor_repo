...
public class TLogicalPeopleGroupsUI extends AbstractParentTagSection {
	 ...
	 */
	public TLogicalPeopleGroupsUI(XMLEditor textEditor, Composite parentComposite, Composite parentTagContainer,
		...
		super(textEditor, parentComposite, parentTagContainer, styleBit,
				new String[] { HTEditorConstants.LOGICAL_PEOPLE_GROUP_TITLE, HTEditorConstants.DOCUMENTATION_TITLE },
	...
	public void onBtnRemove(XMLEditor textEditor) throws JAXBException {
		...
		transition.refreshChildren(HTEditorConstants.LOGICAL_PEOPLE_GROUPS_TITLE, getCompositeIndex(),
				getObjectIndex());
	...
	public void onPageRefresh(XMLEditor editor) throws JAXBException {
		if (logicalPeopleGroups != null) {
			...
			ArrayList<TDocumentation> documentationGroup = (ArrayList<TDocumentation>) logicalPeopleGroups
					.getDocumentation();
			...
			ArrayList<TLogicalPeopleGroup> logicalPeopleGroupGroup = (ArrayList<TLogicalPeopleGroup>) logicalPeopleGroups
					.getLogicalPeopleGroup();
			for (int i = 0; i < logicalPeopleGroupGroup.size(); i++) {
				TLogicalPeopleGroupUI tLogicalPeopleGroupUI = new TLogicalPeopleGroupUI(editor, detailArea, this,
						SWT.NONE, logicalPeopleGroupGroup.get(childObjectIndexes[1]), childObjectIndexes[1],
						childCompositeIndex);
				tLogicalPeopleGroupUI.initialize(editor);
				childComposites.add(childCompositeIndex, tLogicalPeopleGroupUI);
				childCompositeIndex++;
	...
	public void onCreateNewChild(String selection, ScrolledComposite sc3, XMLEditor editor, Composite composite)
		...
		if (selection.equalsIgnoreCase(HTEditorConstants.DOCUMENTATION_TITLE)) {
			...
			logicalPeopleGroups.getLogicalPeopleGroup().add(childObjectIndexes[1], tLogicalPeopleGroup);
			TLogicalPeopleGroupUI tLogicalPeopleGroupUI = new TLogicalPeopleGroupUI(editor, composite, this, SWT.NONE,
					tLogicalPeopleGroup, childObjectIndexes[1], childCompositeIndex);
			childComposites.add(childCompositeIndex, tLogicalPeopleGroupUI);
			childObjectIndexes[1]++;
			childCompositeIndex++;
		}
		...
		if (itemName.equalsIgnoreCase(HTEditorConstants.DOCUMENTATION_TITLE)) {
			...
			for (Composite compositeInstance : childComposites) {
				...
				if (compositeInstance instanceof TDocumentationUI) {
					...
					TLogicalPeopleGroupUI tLogicalPeopleGroupUI = (TLogicalPeopleGroupUI) compositeInstance;
	...
	public void loadModel(Object model) throws JAXBException {
		...
		for (Composite compositeInstance : childComposites) {
			if (compositeInstance instanceof TDocumentationUI) {
				TDocumentationUI tDocumentationUI = (TDocumentationUI) compositeInstance;
				tDocumentationUI
						.loadModel(logicalPeopleGroups.getDocumentation().get(tDocumentationUI.getObjectIndex()));
			} else if (compositeInstance instanceof TLogicalPeopleGroupUI) {
				TLogicalPeopleGroupUI tLogicalPeopleGroupUI = (TLogicalPeopleGroupUI) compositeInstance;
				tLogicalPeopleGroupUI.logicalPeopleGroup = logicalPeopleGroups.getLogicalPeopleGroup()
						.get(tLogicalPeopleGroupUI.getObjectIndex());
				tLogicalPeopleGroupUI.onPageRefresh(textEditor);
				tLogicalPeopleGroupUI.loadModel(
						logicalPeopleGroups.getLogicalPeopleGroup().get(tLogicalPeopleGroupUI.getObjectIndex()));
				this.layout();
			}
		}
	}

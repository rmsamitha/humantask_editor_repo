...
/**
 * The UI class representing the "potentialOwnerAssignment" xml element in the .ht file
...
public class TPotentialOwnerAssignmentsUI extends AbstractParentTagSection {
	 ...
	 */
	public TPotentialOwnerAssignmentsUI(XMLEditor textEditor,
			Composite parentComposite, Composite parentTagContainer,
			int styleBit, Object modelParent, int objectIndex,
			int compositeIndex,String type,QName qname) throws JAXBException {
		super(textEditor, parentComposite, parentTagContainer, styleBit,
				new String[] { HTEditorConstants.DOCUMENTATION_TITLE,
						HTEditorConstants.FROM_TITLE,HTEditorConstants.SEQUENCE_TITLE,HTEditorConstants.PARALLEL_TITLE },
						type);
	...
	public void btnRemoveHandleLogic(XMLEditor textEditor) throws JAXBException {
		...
		abstractParentTagSection.refreshChildren(
				HTEditorConstants.GENERIC_HUMAN_ROLE_TITLE,
	...
	public void refreshLogic(XMLEditor editor) throws JAXBException {
		...
		if (potentialOwnerAssignment.getFrom() != null) {
			TFrom fromObject = (TFrom) potentialOwnerAssignment.getFrom();
			TFromUI tFromUI = new TFromUI(editor,
					detailArea, childCompositeIndex, childObjectIndexes[1],
					SWT.NONE, this, fromObject);
			...
			TParallelUI tParallelUI = new TParallelUI(editor,
                    detailArea,this,SWT.NONE,potentialOwnerAssignment.getParallel(), childObjectIndexes[2], childCompositeIndex);
		...
		if (potentialOwnerAssignment.getSequence() != null) {
			TSequence tSequence = (TSequence) potentialOwnerAssignment.getSequence();
			TSequenceUI tSequenceUI = new TSequenceUI(
	...
	public void newButtonLogic(String selection, ScrolledComposite sc3,
		...
		if (selection.equalsIgnoreCase(HTEditorConstants.DOCUMENTATION_TITLE)) {
            ...
            potentialOwnerAssignment.getDocumentation().add(childObjectIndexes[0],
                    tDocumentation);
            TDocumentationUI tDocumentationUI = new TDocumentationUI(editor,
            ...
            childObjectIndexes[1]++;
				...
				TParallelUI tPriorityExprUI = new TParallelUI(editor,
                     detailArea,this,SWT.NONE,tParallel, childObjectIndexes[2], childCompositeIndex);
				childComposites.add(childCompositeIndex, tPriorityExprUI);
				childObjectIndexes[2]++;
				childCompositeIndex++;
			
		} else if (selection
				.equalsIgnoreCase(HTEditorConstants.SEQUENCE_TITLE)) {
				...
				TSequenceUI tPresentationElementsUI = new TSequenceUI(
			...
			for (Composite compositeInstance : childComposites) {
				...
				if (compositeInstance instanceof TDocumentationUI) {
					...
					if (tFromUI.getCompositeIndex() > childCompositeIndex) {
						tFromUI.setCompositeIndex(tFromUI
								.getCompositeIndex() - 1);
					}
					...
					if (tSequenceUI.getCompositeIndex() > childCompositeIndex) {
						tSequenceUI.setCompositeIndex(tSequenceUI
								.getCompositeIndex() - 1);
					}
					...
					if (tParallelUI.getCompositeIndex() > childCompositeIndex) {
						tParallelUI.setCompositeIndex(tParallelUI
								.getCompositeIndex() - 1);
					}
			...
			for (Composite compositeInstance : childComposites) {
				if (compositeInstance instanceof TDocumentationUI) {
					...
					if (tFromUI.getCompositeIndex() > childCompositeIndex) {
						...
						tFromUI.setObjectIndex(tFromUI
								.getObjectIndex() - 1);
					}
			...
			for (Composite compositeInstance : childComposites) {
				if (compositeInstance instanceof TDocumentationUI) {
					...
					if (tSequenceUI.getObjectIndex() > childObjectIndex) {
						tSequenceUI.setObjectIndex(tSequenceUI
								.getObjectIndex() - 1);
					}
			...
			for (Composite compositeInstance : childComposites) {
				if (compositeInstance instanceof TDocumentationUI) {
					...
					if (tParallelUI.getObjectIndex() > childObjectIndex) {
						tParallelUI.setObjectIndex(tParallelUI
								.getObjectIndex() - 1);
					}
	...
	public void loadModel(Object model) throws JAXBException {
		...
		for (Composite compositeInstance : childComposites) {
			if (compositeInstance instanceof TDocumentationUI) {
				TDocumentationUI tDocumentationUI = (TDocumentationUI) compositeInstance; 
				tDocumentationUI.loadModel(potentialOwnerAssignment.getDocumentation().get(
				...
				TParallelUI tParallelUI = (TParallelUI) compositeInstance; 
				tParallelUI.tParallel = (TParallel) potentialOwnerAssignment.getParallel();
				tParallelUI.refreshLogic(textEditor);
				tParallelUI.loadModel(potentialOwnerAssignment.getParallel());
			} else if (compositeInstance instanceof TSequenceUI) {
				TSequenceUI tSequenceUI = (TSequenceUI) compositeInstance;
				tSequenceUI.tSequence = (TSequence) potentialOwnerAssignment.getSequence();
				tSequenceUI.refreshLogic(textEditor);

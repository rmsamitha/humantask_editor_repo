...
public class TResultUI extends AbstractParentTagSection {
	 ...
	 */
	public TResultUI(XMLEditor textEditor, Composite parentComposite,
	...
	public void btnRemoveHandleLogic(XMLEditor textEditor) throws JAXBException {
		...
		parentContainer.refreshChildren(HTEditorConstants.RESULT_TITLE,
	...
	public void refreshLogic(XMLEditor editor) throws JAXBException {
		...
		ArrayList<TDocumentation> documentationGroup = (ArrayList<TDocumentation>) result
				.getDocumentation();
		...
		for (int i = 0; i < aggregateOrCopyGroup.size(); i++) {
			...
			TAggregateUI tAggregateUI = new TAggregateUI(
	...
	public void newButtonLogic(String selection, ScrolledComposite sc3,
		...
		if (selection.equalsIgnoreCase(HTEditorConstants.DOCUMENTATION_TITLE)) {
			...
			childCompositeIndex++;
				...
				.equalsIgnoreCase(HTEditorConstants.AGGREGATE_TITLE)) {
			...
			TAggregateUI tAggregateUI = new TAggregateUI(editor, composite,
					this, SWT.NONE, tAggregate, childObjectIndexes[1],
					childCompositeIndex);
			childComposites.add(childCompositeIndex, tAggregateUI);
			childObjectIndexes[1]++;
			childCompositeIndex++;
		} else if (selection
				.equalsIgnoreCase(HTEditorConstants.COPY_TITLE)) {
				...
				TCopyUI tCopyUI = new TCopyUI(
						editor, composite, this, SWT.NONE, tCopy,
						childObjectIndexes[1], childCompositeIndex);
				childComposites.add(childCompositeIndex, tCopyUI);
				childObjectIndexes[1]++;
				childCompositeIndex++;
			...
			for (Composite compositeInstance : childComposites) {
				...
				if (compositeInstance instanceof TDocumentationUI) {
					...
					if (tAggregateUI.getCompositeIndex() > childCompositeIndex) {
						tAggregateUI.setCompositeIndex(tAggregateUI
								.getCompositeIndex() - 1);
					}
			...
			for (Composite compositeInstance : childComposites) {
				if (compositeInstance instanceof TDocumentationUI) {
					...
					if (tAggregateUI.getObjectIndex() > childObjectIndex) {
						tAggregateUI.setObjectIndex(tAggregateUI
								.getObjectIndex() - 1);
					}
				} else if (compositeInstance instanceof TCopyUI) {
					TCopyUI tCopyUI = (TCopyUI) compositeInstance;
					...
					if (tCopyUI.getObjectIndex() > childObjectIndex) {
						tCopyUI.setObjectIndex(tCopyUI
								.getObjectIndex() - 1);
					}
				} else {
				}
	...
	public void loadModel(Object model) throws JAXBException {
		...
		for (Composite compositeInstance : childComposites) {
			if (compositeInstance instanceof TDocumentationUI) {
				TDocumentationUI tDocumentationUI = (TDocumentationUI) compositeInstance;
				tDocumentationUI.loadModel(result.getDocumentation().get(
						tDocumentationUI.getObjectIndex()));
			} else
			if (compositeInstance.getClass() == TAggregateUI.class) {
				TAggregateUI tAggregateUI = (TAggregateUI) compositeInstance;
				tAggregateUI.aggregate=(TAggregate) result.getAggregateOrCopy().get(tAggregateUI.getObjectIndex());
				tAggregateUI.refreshLogic(textEditor);
				tAggregateUI.loadModel(result.getAggregateOrCopy().get(
						tAggregateUI.getObjectIndex()));
			} else
			if (compositeInstance.getClass()==  TCopyUI.class) {
				
				TCopyUI tCopyUI = (TCopyUI) compositeInstance;
				tCopyUI.tcopy=(TCopy) result.getAggregateOrCopy().get(tCopyUI.getObjectIndex());
				tCopyUI.refreshLogic(textEditor);
				tCopyUI.loadModel(result.getAggregateOrCopy()
						.get(tCopyUI.getObjectIndex()));
			}
		}
	}
